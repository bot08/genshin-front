<template>
  ...
  <div class="grid grid-cols-1 sm:grid-cols-1 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5">
    <!-- Card -->
    <div v-for="item in another" :key="item.name" class="rounded-lg mt-1 mb-4 md:mb-8 overflow-hidden shadow-lg bg-gray-50 dark:bg-gray-700 transition-colors sm:mx-auto sm:justify-center sm:text-center sm:w-48">
        <router-link class="flex sm:block" v :to="item.href">
            <img class="w-20 h-20 sm:w-24 sm:h-24 sm:mt-3 sm:m-auto rounded-xl mx-3 my-3 sm:mb-0" v :src="item.ico" alt="char-portret">
            <div class="px-3 py-1 my-auto">
                <h1 class="font-bold mb-1 sm:mt-1 sm:mb-2 text-xl dark:text-gray-100">{{ item.name }}</h1>
            </div>
        </router-link>
    ...
    <div v-for="n in 4" :key="n" class="rounded-lg mt-1 mb-4 md:mb-8 overflow-hidden shadow-lg bg-gray-50 dark:bg-gray-700 transition-colors sm:mx-auto sm:justify-center sm:text-center sm:w-48">
        <div class="flex sm:block">
            ...
            <div class="px-3 py-1 my-auto">
                <p class="h-6 my-4 bg-gray-200 dark:bg-gray-600 rounded w-28 sm:mx-auto animate-pulse"></p>
            </div>
        </div>
    </div>
  </div>
...
const another = [
  { name: 'Еда', href: '/food', ico: "/img/icon_food.png" },
  { name: 'Крафты', href: '/blacksmith', ico: "/img/icon_blacksmith.png" },
  { name: 'Алхимия', href: '/alchemy', ico: "/img/icon_alchemy.png" },
]
