<template>
    ...
    <SelectMenu :sortProps='sort' @updContent='getContent'/>
      ...
      <div v-if="loading" class="grid grid-cols-1 lg:grid-cols-2 mt-1">
        <div v-for="n in 6" :key="n" class="px-3 pb-2 rounded-lg mb-4 md:mb-8 overflow-hidden shadow-lg bg-gray-50 dark:bg-gray-700 transition-colors sm:mx-4">
      ...
      <div v-else class="grid grid-cols-1 lg:grid-cols-2 mt-1">
        <div v-for="item in banners" :key="item.name" class="px-3 pb-2 rounded-lg mb-4 md:mb-8 overflow-hidden text-gray-900 dark:text-gray-200 shadow-lg bg-gray-50 dark:bg-gray-700 transition-colors sm:mx-4">
            ...
            <h3 class="pb-1 pt-2 ml-2 text-2xl font-bold leading-9 tracking-tight text-gray-900 sm:leading-10 dark:text-gray-200">
              ...
              <p v-if="item.ch5star" class="ml-2 text-xl dark:text-gray-100">{{ item.ch5star }} 5<StarIcon class="h-5 w-5 inline v-align-min3-5"/></p>
                <p v-if="item.ch4star1" class="ml-2 text-xl dark:text-gray-100">{{ item.ch4star1 }} 4<StarIcon class="h-5 w-5 inline v-align-min3-5"/></p>
                <p v-if="item.ch4star2" class="ml-2 text-xl dark:text-gray-100">{{ item.ch4star2 }} 4<StarIcon class="h-5 w-5 inline v-align-min3-5"/></p>
                <p v-if="item.ch4star3" class="ml-2 mb-1 text-xl dark:text-gray-100">{{ item.ch4star3 }} 4<StarIcon class="h-5 w-5 inline v-align-min3-5"/></p>
        </div>
      </div>
