<template>
  ...
  <div v-if="loading" class="grid grid-cols-1 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
      <div v-for="n in 16" :key="n" class="rounded-lg mt-1 mb-4 md:mb-8 overflow-hidden shadow-lg bg-gray-50 dark:bg-gray-700 sm:mx-auto justify-center text-center animate-pulse sm:w-60">
  ...
  <div class="grid grid-cols-1 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
    <div v-for="item in characters" :key="item.name" class="rounded-lg mt-1 mb-4 md:mb-8 overflow-hidden shadow-lg bg-gray-50 dark:bg-gray-700 sm:mx-auto justify-center text-center sm:w-60">
        <router-link v :to="item.href">
            <img class="w-44 h-44 sm:w-28 sm:h-28 m-auto rounded-xl mt-4" v :src="'https://sushicat.pp.ua/api/'+item.ico.path" alt="char-portret">
            <div class="px-1 py-1">
